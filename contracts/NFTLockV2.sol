...
constructor() ERC721("Pledge certificate","Pledge certificate"){

}
function setPowerContract(address _address,bool _set) public onlyOwner{
powerAddr[_address] = _set;
}
function mintCertificate(address _user) external{
    require(powerAddr[msg.sender], "No Permission");
    _mint(_user, _tokenId);
    _tokenId++;
}
function burnCertificate(address _user,uint256 _id) external {
    require(powerAddr[msg.sender], "No Permission");
    require(_ownerOf(_id) == _user,"input correct id");
    _burn(_id);
}
}
contract NFTLock is Ownable,ReentrancyGuard{ 
    ...
    address public NFTcertificatAddr;
    ...
    constructor(address _srt,address _supNft, address _bigNft, address _smallNft,address _NFTcertificat,address _supNftV3,address _bigNftV3,address _smallNftV3) {
        ...
        bigNft = _bigNft;
        smallNft = _smallNft;
        NFTcertificatAddr = _NFTcertificat;
    }
    ...
    function lockNft(uint256 _nft,uint256 _tokenId) public nonReentrant{
        ...
        if(_nft == 1) {
            ...
            INFTcertificat(NFTcertificatAddr).mintCertificate(msg.sender);
